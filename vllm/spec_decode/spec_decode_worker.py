...
class SpecDecodeWorker(LoraNotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def get_seq(self, llm_engine, virtual_engine):
        cached_outputs = llm_engine.cached_scheduler_outputs[virtual_engine]
        seq_group_metadata_list = cached_outputs.seq_group_metadata_list
        scheduler_outputs = cached_outputs.scheduler_outputs
        allow_async_output_proc = cached_outputs.allow_async_output_proc

        ctx = llm_engine.scheduler_contexts[virtual_engine]
        ...
        # batch has completed.
        if not llm_engine._has_remaining_steps(seq_group_metadata_list):
            # Schedule iteration
            (seq_group_metadata_list, scheduler_outputs,
             allow_async_output_proc
             ) = llm_engine.scheduler[virtual_engine].schedule()
            ...
            if (llm_engine.scheduler_config.is_multi_step
                ...
                # lookahead slots
                llm_engine._cache_scheduler_outputs_for_multi_step(
        ...
        if not scheduler_outputs.is_empty():
            finished_requests_ids = llm_engine.scheduler[
            ...
            last_sampled_token_ids = \
                llm_engine._get_last_sampled_token_ids(virtual_engine)
    ...
    def post_process(self, llm_engine, outputs, ctx, scheduler_outputs, seq_group_metadata_list, allow_async_output_proc):
        ...
        if not llm_engine._has_remaining_steps(seq_group_metadata_list):
            ...
            if llm_engine.scheduler_config.is_multi_step:
                llm_engine.cached_scheduler_outputs[0] = SchedulerOutputState()
            ...
            if outputs and allow_async_output_proc:
                ...
                llm_engine._advance_to_next_step(
            ...
            if not allow_async_output_proc:
                ...
                # Log stats.
                llm_engine.do_log_stats(scheduler_outputs, outputs)
        ...
        if not llm_engine.has_unfinished_requests():
            ...
            llm_engine.model_executor.stop_remote_worker_execution_loop()
    ...
    def execute_model_hete_spec_decode(self, llm_engine):
        ...
        while llm_engine.has_unfinished_requests():
            ctx, execute_model_req, scheduler_outputs, seq_group_metadata_list, allow_async_output_proc = self.get_seq(llm_engine, 0)
    ...
    def _verify_tokens(
        ...
        # num_spec_tokens = proposal_token_ids.numel()
        # accept_num = (torch.where(accepted_token_ids > -0.5, 1, 0).sum(-1) - 1).item()
